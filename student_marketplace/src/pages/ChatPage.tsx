...
export default function ChatPage() {
   ...
   const handleSendMessage = () => {
      ...
      sendMessage(activeSession.id, inputValue, 'me');
      setInputValue('');
   };
   ...
   return (
      <div className="h-screen w-full bg-[#F8F9FC] flex flex-col overflow-hidden">
         ...
         <div className="flex-1 flex overflow-hidden relative w-full mx-auto border-x border-gray-100 bg-white shadow-sm">
            {/* SIDEBAR */}
            <div className={`w-full md:w-80 border-r border-gray-100 flex flex-col h-full bg-white flex-shrink-0 absolute md:relative z-20 transition-transform duration-300 ${activeSessionId && window.innerWidth < 768 ? '-translate-x-full' : 'translate-x-0'}`}>
               ...
               <div className="flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-gray-200">
                  {sessions.map(session => (
                     ...
                     >
                        ...
                        >
                           <img src={session.partner.avatar} className="w-12 h-12 rounded-full border border-gray-200 hover:border-teal-500 transition-colors object-cover" alt="" />
                        </Link>
            ...
            {/* CHAT AREA */}
            <div className={`flex-1 flex flex-col h-full bg-[#F8F9FC] min-w-0 relative w-full absolute md:relative transition-transform duration-300 ${!activeSessionId && window.innerWidth < 768 ? 'translate-x-full' : 'translate-x-0'}`}>
               {activeSession ? (
                   <>
                       <div className="h-16 bg-white px-4 border-b border-gray-100 flex items-center justify-between flex-none z-10 shadow-sm">
                          <div className="flex items-center gap-3">
                             <div className="md:hidden cursor-pointer" onClick={() => setActiveSessionId('')}><ArrowLeft size={20} className="text-gray-500" /></div>
                             
                             <Link to={`/profile/${activeSession.partner.id}`} className="flex items-center gap-3 group hover:opacity-80 transition-opacity">
                                <img src={activeSession.partner.avatar} className="w-10 h-10 rounded-full border border-gray-100" alt="" />
                                <div>
                                   <h3 className="font-bold text-gray-900 text-sm md:text-base group-hover:text-teal-600 transition-colors">{activeSession.partner.name}</h3>
                   ...
                   <div className="flex-1 flex items-center justify-center text-gray-400">Chọn một đoạn chat để bắt đầu</div>
               )}
            </div>
         </div>
      </div>
