...
import { CHAT_SESSIONS as MOCK_SESSIONS } from '../mock/data';
             ...
             <div className="flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-gray-200">
                {MOCK_SESSIONS.map(session => (
                   <div 
                     ...
                     className={`p-3 rounded-xl flex items-center gap-3 cursor-pointer transition-all mb-1 ${
                        session.id === activeSessionId ? 'bg-violet-50' : 'hover:bg-gray-50'
                     }`}
                   >
                   ...
                   return (
                      <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                         <div className={`max-w-[75%] p-3 rounded-2xl text-sm shadow-sm ${
                            isMe ? 'bg-gradient-to-br from-violet-600 to-pink-500 text-white' : 'bg-white text-gray-800 border border-gray-100'
                         }`}>
                            {msg.text}
                         </div>
                      </div>
